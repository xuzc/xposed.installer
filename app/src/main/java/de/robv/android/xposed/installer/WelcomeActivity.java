...
import de.robv.android.xposed.installer.update.ApkUpdateManager;
import de.robv.android.xposed.installer.util.AppUtil;
import de.robv.android.xposed.installer.util.CommonDataUtil;
import de.robv.android.xposed.installer.util.Constant;
...
import de.robv.android.xposed.installer.util.NetUtil;
...
public class WelcomeActivity extends XposedBaseActivity implements NavigationView.OnNavigationItemSelectedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化了模块数据。里边顺便把白名单的数据加入进来。
        ModuleUtil.getInstance().addListener(this);
        mRepoLoader.addListener(this);
        notifyDataSetChanged();
        //第一次启动或者页面不存在的启动
        restartDevice(getIntent());
    }

    /**
     * singleTask页面存在接受到意图，要重新设置一下[页面存在时候，启动]
     */
    @Override
    protected void onNewIntent(Intent intent) {
        super.onNewIntent(intent);
        restartDevice(intent);
    }
...
//                return;
    ...
    /**
     *  xposed版本和cyt版本有变化的时候，都会触发关机重启的
     **/
    private void restartDevice(Intent intent) {
        //设置白名单和选中
        ModuleUtil.getInstance().initSelectAllWhite();

        //1.cyt升级或者新增，通知xposed的重启的
        if (isNeedRestartByIntent(intent)) {
            String packageName = intent.getStringExtra(Constant.INTENT_PACKAGENAME);
            showFloat();
            LogUtil.e(TAG, "packageName:" + packageName);
        } else if (isXposedChange()) {
            //2.xposed新增的或者升级
            showFloat();
        }
    }

    private boolean isNeedRestartByIntent(Intent intent) {
        if (intent != null) {
            String action = intent.getAction();
            if (!TextUtils.isEmpty(action) && action.equals(Constant.CYT_CHANGE_ACTION)) {
                return true;
            }
        }
        return false;
     ...
     * 新增或者升级
     */
    private boolean isXposedChange() {
        int fileCode = sharedPreferencesHelper.getXposedAppCode();
        Log.e(TAG, "fileCode"+fileCode);
        int curCode = AppUtil.getAppCode(getApplicationContext());
        if (fileCode <= 0 || (fileCode > 0 && curCode > fileCode)) {
            return true;
        }
        return false;
    }
    ...
    private void showFloat() {
        FloatManager floatManager = new FloatManager(getApplicationContext());
    ...
    private void checkUpdate() {
        if (NetUtil.isNetworkAvailable(this)) {
            ...
            apkUpdateManager.updateCheck();
        } else {
            Toast.makeText(this, R.string.global_no_net, Toast.LENGTH_SHORT).show();
        }
    }

    public void testData(){
        CommonDataUtil.readData(getApplicationContext());
    }
