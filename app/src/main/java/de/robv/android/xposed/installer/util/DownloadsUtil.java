...
public class DownloadsUtil {
    ...
    public static SyncDownloadInfo downloadSynchronously(String url, File target) {
        ...
        try {
            if (NetUtil.isNetworkAvailable(XposedApp.getInstance())) {
                ...
                if (! new CopyFromAssetsUtil.Builder(XposedApp.getInstance())
                        .setUrl(url)
                        .setAssetsName("framework.json")
                        .setDestination(target)
                        .setCallback(new CopyFromAssetsUtil.CopyFinishedCallback() {
                            @Override
                            public void onCopyFinished(Context context, CopyFromAssetsUtil.Builder b) {
//                                mPref.edit()
//                                        .putString("download_" + b.getmUrl() + "_modified", null)
//                                        .putString("download_" + b.getmUrl() + "_etag", null).apply();
                            }
                        })
                        .copy()) {
                    return new SyncDownloadInfo(SyncDownloadInfo.STATUS_FAILED,
                            mApp.getString(R.string.repo_download_failed, url,
                                    "copy from assets failed"));
                }
            }
