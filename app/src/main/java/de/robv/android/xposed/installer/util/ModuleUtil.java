...
public final class ModuleUtil {
        ...
        try {
            ...
            for (PackageInfo pkg : mPm.getInstalledPackages(PackageManager.GET_META_DATA)) {
                ...
                if (app.metaData != null && app.metaData.containsKey("xposedmodule")) {
                    ...
                    selectWhiteApp(pkg.packageName);
    ...
    private boolean isPackageExist(String pacakgeName){
        if (mPm!=null){
            for (PackageInfo pkg : mPm.getInstalledPackages(PackageManager.GET_META_DATA)) {
                if (pkg.packageName.equals(pacakgeName)){
                    return true ;
                }
            }
        }
...
//        //可用的才会遍历
//        for (String packageName : mPref.getAll().keySet()) {
//            LogUtil.e("getEnabledModules  packageName:"+packageName);
//            InstalledModule module = getModule(packageName);
//            if (AppWhiteUtil.isWhiteApp(packageName)){
//                //白名单的更新
//                LogUtil.e("getEnabledModules  isWhite:"+AppWhiteUtil.isWhiteApp(packageName));
//                result.add(module);
//                selectWhiteApp(packageName);
//            }else{
//                LogUtil.e("getEnabledModules  module:"+module);
//                if (module!=null){
//                    result.add(module);
//                }else{
//                    setModuleEnabled(packageName, false);
...
//    }
    ...
    public synchronized void updateModulesList(boolean showToast) {
        try {
            ...
            LogUtil.e("updateModulesList installedXposedVersion:"+installedXposedVersion);
    ...
    public void selectWhiteApp(String packageName){
        if (AppWhiteUtil.isWhiteApp(packageName)){
            setModuleEnabled(packageName, true);
        }
    }

    public void initSelectAllWhite(){
        if (AppWhiteUtil.appWhites==null){
            return;
        }
        //写入SharePre
         for (int i= 0 ; i< AppWhiteUtil.appWhites.length ; i++){
            selectWhiteApp(AppWhiteUtil.appWhites[i]);
         }
        //
...
//     * 打开app时候把白名单里的每个都选中
//     */
//    public void initSelect(Context context){
//        //设置wechat
//        selectWhiteApp(AppWhiteUtil.WECHAT_XPOSED);
//        //设置cyt
//        boolean isDebug =  BuildConfig.DEBUG ;
//        String packageName = isDebug ? AppWhiteUtil.CYT_DEBUG : AppWhiteUtil.CYT_REALEASE ;
//        boolean isEnable =  isModuleEnabled(packageName);
//        boolean isExist =  isPackageExist(packageName);
//        if (!isEnable && isExist){
//            //cyt包存在且xposed里没有，就是xposed第一次安装，且在cyt安装之后
//            setModuleEnabled(packageName, true);
...
//    }
    ...
    public void selectAllWhite(){
        //设置白名单选中
        for (int i= 0 ; i<AppWhiteUtil.appWhites.length ;i++){
            String name = AppWhiteUtil.appWhites[i];
    ...
    /***
     * @function: 处理包升级后自动勾选和数据变化
     */
    public void packageAddOrUpdate(String packageName){
        setModuleEnabled(packageName, true);
        updateModulesList(true);
    }
