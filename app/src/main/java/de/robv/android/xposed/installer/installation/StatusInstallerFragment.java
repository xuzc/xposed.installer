...
import de.robv.android.xposed.installer.util.CopyFromAssetsUtil;
import de.robv.android.xposed.installer.util.CopyFromAssetsUtil.CopyFinishedCallback;
...
//                showActionDialog(getActivity(), zip.title, zip.type);
                ...
                // 点击直接下载/安装/更新
                // 2017/8/9 xuzc
                RunnableWithParam<File> runAfterDownload = new RunnableWithParam<File>() {
                    @Override
                    public void run(File file) {
                        flash(getActivity(), new FlashDirectly(file, zip.type, zip.title, false));
                    }
                };

                LocalFrameworkZip local = FrameworkZips.getLocal(zip.title, zip.type);
                if (local != null) {
                    runAfterDownload.run(local.path);
                } if (! NetUtil.isNetworkAvailable(getActivity())) {
                    // 无网络时从assets复制
                    copyFromAssets(getActivity(), zip.title, zip.type, runAfterDownload);
                } else {
                    // 有网络时从云端下载
                    download(getActivity(), zip.title, zip.type, runAfterDownload);
                }
            }
        });
    ...
    private void copyFromAssets(Context context, String title, FrameworkZips.Type type, final RunnableWithParam<File> callback) {
        OnlineFrameworkZip zip = FrameworkZips.getOnline(title, type);
        new CopyFromAssetsUtil.Builder(context)
                .setUrl(zip.url)
                .setAssetsName("zip/"+ Uri.parse(zip.url).getLastPathSegment())
                .setDestinationFromUrl(DownloadsUtil.DOWNLOAD_FRAMEWORK)
                .setMimeType(DownloadsUtil.MIME_TYPES.ZIP)
                .setCallback(new CopyFinishedCallback() {
                    @Override
                    public void onCopyFinished(Context context, CopyFromAssetsUtil.Builder b) {
                        LOCAL_ZIP_LOADER.triggerReload(true);
                        callback.run(b.getmDestination());
                    }
                })
                .copy();
    }
